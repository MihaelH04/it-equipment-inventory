...
public class SitesController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> Index(string searchString, string sortBy, string sortDir, string statusFilter, int page = 1, int pageSize = PaginationConstants.DefaultPageSize)
    {
        var result = await GetFilteredSitesAsync(searchString, sortBy, sortDir, statusFilter, page, pageSize);
    ...
    [HttpGet]
    public async Task<IActionResult> IndexTable(string searchString, string sortBy, string sortDir, string statusFilter, int page = 1, int pageSize = PaginationConstants.DefaultPageSize)
    {
        var result = await GetFilteredSitesAsync(searchString, sortBy, sortDir, statusFilter, page, pageSize);
        SetSiteListViewBags(searchString, sortBy, sortDir, statusFilter);
    ...
    private async Task<PagedResult<Site>> GetFilteredSitesAsync(
        string searchString, string sortBy, string sortDir, string statusFilter, int? page, int pageSize = PaginationConstants.DefaultPageSize)
    {
        ...
        if (page is null)
            return new PagedResult<Site> { Items = await query.ToListAsync(), CurrentPage = 1, TotalPages = 1, TotalCount = totalCount, PageSize = pageSize };
        ...
        return new PagedResult<Site> { Items = items, CurrentPage = currentPage, TotalPages = totalPages, TotalCount = totalCount, PageSize = pageSize };
    }
